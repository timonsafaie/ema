...
class Latex {
  ...
  public function getLatex($mxs) {
	  ...
	  for($i = 0; $i < $mxslen; $i++) {
		  ...
		  if (is_numeric($mxs[$i]) || ($mxs[$i] == '.') || ($mxs[$i] == '0')) {
			  ...
			  // Advance pointer
			  $i += strlen($number)-1;
              // Deactivate alpha specification
              $isAlpha = false;
		  }
			  ...
			  if ($mxs[$i] == "\\") {
				  if (in_array($mxs[$i+1], $openenclosure)) {
					  ...
					  if (!empty($completeenclosure)) {
                          ...
                          // Check to see if we have angled brackets, pipes, or just a pipe
                          $leftencl = $completeenclosure[1];
                          $rightencl = $completeenclosure[strlen($completeenclosure)-1];
                          /*
                          if ($leftencl == "<") {
                              $leftencl = "\\langle";
                          }
                          ...
                          */
                          ...
                          $leftside = $left.$leftencl;
                          $middle = $this->getLatex(substr($completeenclosure, 2, strlen($completeenclosure) - 4));
                          $rightside = $right.$rightencl;
						  $element = $leftside." ".$middle." ".$rightside;
					  }
				  } else {
		  ...
		  /*
          // If an element is a letter,
          // collect all the consecutive 
          // letters behind it to see
          // if the word is should be 
          // treated as a reserved word.
          if($isAlpha) {
            $romanized = $this->getRomanization($word);
            if (!empty($romanized)) {
                $latex .= $romanized." ";
                $word = "";
                $isAlpha = false;
          ...
          */
          if (!$isAlpha) {
              if (!empty($word)) {
                  $latex .= $this->getAllRomanization($word)." ";
              }
              // Once an element is retrieved
              // and converted to latex code
              // add it to the latex expression.
      ...
      // Corner Case: Make sure the storedup
      // var is also included
      if (!empty($word)) {
        $latex .= $this->getAllRomanization($word);
      }
  ...
  // Romanize certain keywords like sin, cos, ln
  private function getRomanization($text) {
      $romans = array('sin','cos','tan','sec','csc','cot','sinh','cosh','tanh','sech','csch','coth','arcsin','arccos','arctan','arcsec','arccsc','arccot','arcsinh','arccosh','arctanh','arcsech','arccsch','arccoth','log','ln','mod','gcd','lcm','min','max','inf','sup','det','Tr');
      $result = "";
      if (in_array($text, $romans)) {
          $result = $text;
      }
      return $result;
  }

  // Format all of the texts romanization if such exist
  private function getAllRomanization($text) {
      $result = "";
      $globalIndex = 0;
      while ($globalIndex < strlen($text)) {
          $localIndex = $globalIndex;
          $candidate = $processed = $accepted = "";
          while ($localIndex < strlen($text)) {
            $candidate .= $text[$localIndex];
            $processed = $this->getRomanization($candidate);
            if (!empty($processed)) 
                $accepted = $processed;
            $localIndex++;
          }
          if (!empty($accepted)) {
              $result .= "\\".$accepted." ";
              $globalIndex += strlen($accepted);
          } else {
              $result .= $text[$globalIndex];
              $globalIndex++;
          }
      }
      return $result;
